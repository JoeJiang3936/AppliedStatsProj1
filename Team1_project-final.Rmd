...
author: "Applied Statistics Project 1 Team 1"
date: "June, 2019"
output:
...
**Introduction** 
--------------------------------------------
One of the biggest pitfalls for building prediction models is the risk of overfitting the training dataset, resluting in poor prediction accuracy of the model. The reason for overfitting is that with the increased number of predictors, the model not only fits the signal, but also tried to fit the noise in the trainign data. Mainly, there are two ways to prevent overfitting, validation/cross-validation and regularization. The idea of validation is very straight forward. Before fitting a model, a small portion of training data is randomly setted aside and used for evaluating the accuracy of the model trained on the rest of data. any model with low training error, but has very high testing error is a typical overfitted model. In general, complicated models with many predictors are at higher risk of being overfitting. Thus, the key to prevent overfitting the training data is to keep the model simple (less predictors) and make the weights (coefficients) of predictors small. One popular way to do that is by including  a regularization term in the model. Two shrinkage approaches, Ridge and lasso,  are the most popular regularization methods (Figure 1).  


<center>__Ridge model :   minimize (RSS + $\lambda\sum_{j=1}^p (\beta_j)^2$)__  <center> 


\center__Lasso model:    minimize (RSS + $\lambda\sum_{j=1}^p |\beta_j|)$__  \center
  
*Figure 1. Ridge and lasso models. L2 and L1 penalties are introduced into the ridge and lasso models respectively. \lambda is the tuning parameter, usually determined by cross validation.*   


In the ridge model, a L2 regularization term $\lambda\sum_{j=1}^p (\beta_j)^2$ (also called L2 norm or ridge penalty), is introduced into the model to keep the coefficients from getting too large. Similarly, for the lasso model, a L1 regularization term $\lambda\sum_{j=1}^p |\beta_j|)$ (L1 norm or L1 penalty), is included in the model to force some of the coefficients to zero. In this sense, these two models achieved at least two objectives, one is to keep the weights of predictors from getting too large, thus lower the variation of prediction. The other is that both approaches essentially perform the feature selection, force the weights of less important predictor close or equal to zero.  

To choose the proper penalty terms for both ridge and lasso models, validation or cross-validation are used to find the lambda resulting in least amount of validation errors. In particular, cross validation is popular because it uses all the training dataset for model training and in addition most packages will automatically choose the best tuning parameter \lambda.  


**Data Description**
-------------------------

The Ames house price dataset on Kaggle (https://www.kaggle.com/c/house-prices-advanced-regression-techniques) is one of the most popular dataset for regression models. It is an updated and modern version of the traditional Boston Housing dataset. The training and testing (for competition) datasets each have data on about 1,500 houses sold from 2006 to 2010 in Ames, IA. Besides the response variable, SalePrice, the datasets contain about 80 predictive variables associated with the price of a typical house, such as its location (Neighborhood), size (Gross living area), quality and condition (OverallQual and OverallCond). And the predictors are a rich mixture of both numerical and categorical variables. The purpose of the project is to generate several predictive linear regression models, including OLS (ordinary least square model) and OLS with regularization (ridge and lasso models), and compare them in terms of predictive accuracy).
...
###5. Generate correlation martix and list and remove highly correlated variables
...
###7. looking for outliers in train dataset

__We first idenitfied the predictors that are highly correlated with the response variable SalePrice and plotted one of strong predictor GrLivArea against SalePrice and visually identified two datapoints (id =524 and 1299) with high leverage and influence (highlighted in red). We further fitted a preliminary full linear regression model and examined its residual vs leverage plot. Indeed, the same two data points have very large residuals and high cook's D score (notice that Id= 1299 is actually literally off the chart). In addition, further investigation confirmed that both are presales and likely not reflective their true sale price. We removed these two outliers that have the largest living area (GrLivArea > 4500), but with exceptionally low sale prices.__  
...
__After extensive data cleanup and transformation, we are ready to train regression models. We are going to train three models: a regular linear regression model, also called ordinary least square (OLS) model, ridge regression model and lasso regression model.__  
...
###3. fit the OLS model
__We first fitted the full ordinary least square regression model and assessed its residual plots for assumptions of linear regression. First of all, we have no reason to suspect that all sales are independent from each other and likely reflects its true value. Second, with the exception of houses with low predicted values (fitted values < 11.5), the variance is more or less constant. Third, from the QQ-plot, the assumption of normal distribution of residuals are less than ideal, significant deviation exists in both end of quantiles. Fourth, many predictors are indeed linearly correlated with the response variable, SalePrice. Thus, the linear regression modle is likely a good model for the dataset.__
