...
__Ridge model :   minimize (RSS + $\lambda\sum_{j=1}^p (\beta_j)^2$)__  


__Lasso model:    minimize (RSS + $\lambda\sum_{j=1}^p |\beta_j|)$__  

<left>In the ridge model, a L2 regularization term $\lambda\sum_{j=1}^p (\beta_j)^2$ (also called L2 norm or ridge penalty), is introduced into the model to keep the coefficients from getting too large. Similarly, for the lasso model, a L1 regularization term $\lambda\sum_{j=1}^p |\beta_j|)$ (L1 norm or Lasso penalty), is included in the model to force some coefficients to zero. In this sense, these two models achieved at least two objectives, one is to keep the weights of predictors from getting too large, thus lowering the variation of prediction. The other is that both approaches essentially perform feature selection by forcing the weights of less important predictors close or equal to zero.  
